...
export class FormCategoryService {
  ...
  async findAll(filters?: { isActive?: boolean; search?: string; parentId?: number; tenantId?: number; projectId?: number }) {
    const where: any = {};
    if (filters?.isActive !== undefined) where.isActive = filters.isActive;
    if (filters?.parentId !== undefined) where.parentId = filters.parentId;
    // Strict tenant+project filter: only categories that belong to this tenant+project
    ...
    return this.prisma.formCategory.findMany({
      ...
      include: {
        tenant: { select: { id: true, name: true, tenant_ID: true } },
        project: { select: { id: true, name: true, project_ID: true, code: true } },
      },
    });
  }
