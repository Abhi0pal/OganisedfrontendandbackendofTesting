...
const SERVICE_ID = '969.0';

async function main() {
  console.log('==========================================================');
  console.log(` SEEDING: Tree Permission Workflow for Tenant ${TENANT_ID}`);
  console.log('==========================================================\n');
  ...
  const nodesData = [
    { code: 'P_001', name: 'Level 0: Investor Submit', type: 'START', role: 'Investor', x: 100, y: 100 },
    ...
    { code: 'P_008', name: 'Level 7: Tree Officer TO Gen', type: 'STANDARD', role: 'Tree-Officer', x: 1500, y: 100 },
    { code: 'P_009', name: 'Level 8: Investor Compliance', type: 'STANDARD', role: 'Investor', x: 1700, y: 100 },
    { code: 'P_010', name: 'Level 9: Garden Inspector Verification', type: 'STANDARD', role: 'Garden-Inspector', x: 1900, y: 100 },
    { code: 'P_011', name: 'Level 10: Final Closure', type: 'END', role: 'Investor', x: 2100, y: 100 },
  ];
  ...
  addAction('P_002', 'REVERT', 'Return to Investor', [{ targetProcessCode: 'P_001', label: 'Return', priority: 1 }]);

  // L2
  addAction('P_003', 'FORWARD', 'Forward to Garden Inspector', [{ targetProcessCode: 'P_004', label: 'Forward', priority: 0 }]);
  ...
  // L6
  addAction('P_007', 'APPROVE', 'Approve / Forward', [{ targetProcessCode: 'P_008', label: 'Approve', priority: 0 }]);
  addAction('P_007', 'REJECT', 'Reject Application', [{ targetProcessCode: 'P_011', label: 'Reject', priority: 1 }]);
  addAction('P_007', 'REVERT', 'Return to Addl Commissioner', [{ targetProcessCode: 'P_006', label: 'Return', priority: 2 }]);

  // L7
  addAction('P_008', 'FORWARD', 'Generate TO / Forward', [{ targetProcessCode: 'P_009', label: 'Forward', priority: 0 }], { allowPaymentDemand: true });
  addAction('P_008', 'REVERT', 'Return to Commissioner', [{ targetProcessCode: 'P_007', label: 'Return', priority: 1 }]);

  // L8
  addAction('P_009', 'FORWARD', 'Payment / Compliance Upload', [{ targetProcessCode: 'P_010', label: 'Forward', priority: 0 }]);
  addAction('P_009', 'REVERT', 'Return to Tree Officer', [{ targetProcessCode: 'P_008', label: 'Return', priority: 1 }]);

  // L9
  addAction('P_010', 'FORWARD', 'Verify Plantation / Proof', [{ targetProcessCode: 'P_011', label: 'Forward', priority: 0 }]);
  addAction('P_010', 'REVERT', 'Return to Investor', [{ targetProcessCode: 'P_009', label: 'Return', priority: 1 }]);

  // L10 (END node) - No forward transitions
  ...
  const wfDef = await prisma.workflowConfiguration.create({
    data: {
      ...
      code: wfCode,
      name: 'Tree Permission / TO Workflow',
      description: '11-Step Tree Permission and TO Workflow process',
      version: 1,
      status: 'PUBLISHED',
