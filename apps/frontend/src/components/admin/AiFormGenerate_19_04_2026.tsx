            ...
            {workflowJson && showWorkflow && (
              <>
                ...
                <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 13, color: '#64748b' }}>
                  <span><i className="bi bi-list-ol" style={{ marginRight: 4 }} />{(workflowJson as any).workflow_steps?.length ?? 0} workflow steps</span>
                  <span><i className="bi bi-file-earmark-text" style={{ marginRight: 4 }} />{(workflowJson as any).officer_forms?.length ?? 0} officer forms</span>
                </div>
