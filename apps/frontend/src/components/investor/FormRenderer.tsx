...
export function FormRenderer({
            ...
            if (field.input_type !== 'addmore' && field.input_type !== 'button') {
                ...
                field.add_more_groups?.forEach((g: any) => {
                    ...
                    rows.forEach((row: any, rIdx: number) => {
                        const isRowEmpty = g.columns.every((col: any) => isEmptyValue(row[col.field_code]));
                        const isOptionalEmptyRow = !g.min_rows && isRowEmpty;
                        if (isOptionalEmptyRow) return;
                        ...
                        g.columns.forEach((col: any) => {
                            ...
                            if (col.is_required === 'Y' && isEmptyValue(val)) {
                                valid = false;
                                newErrors[fieldKey] = `${col.label || 'This field'} is required`;
    ...
    const handlePageNavigation = useCallback((pageIndex: number) => {
        ...
        if (pageIndex > activePageIndex) {
            const canMoveForward = ensurePagesValidThrough(pageIndex - 1);
    ...
    const renderInput = (field: any) => {
        ...
        switch (inputType) {
            case 'textarea': return <InputTextarea className={css} value={val ?? ''} rows={3} onChange={(e) => { const next = e.target.value; if (typeof field.max_length === 'number' && next.length > field.max_length) { return; } handleChange(field.field_code, next); }} disabled={disabled} placeholder={placeholder} />;
            case 'number': {
                ...
                if (maxDigits !== undefined) {
                    return <InputText
                        ...
                        onChange={(e) => {
                            ...
                            handleChange(field.field_code, raw === '' ? null : raw);
                        }}
                        onBlur={() => {
                ...
                return <InputNumber className={css} value={val !== null && val !== undefined && val !== '' ? parseFloat(String(val)) : null} onValueChange={(e) => handleChange(field.field_code, e.value)} disabled={disabled} useGrouping={false} placeholder={placeholder} />;
            }
            case 'select':
                ...
                if (field.field_code === 'UK-FCL-03316_0' || field.field_code === 'UK-FCL-03120_0') {
                    return (
                        <label className="d-flex align-items-center gap-1 cursor-pointer">
                            <Checkbox checked={!!val} onChange={(e) => handleChange(field.field_code, e.checked)} disabled={disabled} />
                    ...
                    <Calendar
                        ...
                        onChange={(e) => {
                            ...
                            if (validationErr) {
                                ...
                                handleChange(field.field_code, null);
                            } else {
                                // Validation passed - set the value normally
                                handleChange(field.field_code, formattedVal);
            ...
            case 'datetime-local': {
                ...
                return <Calendar className={css} value={parseDateValue(val)} onChange={(e) => handleChange(field.field_code, e.value ? format(e.value, 'dd/MM/yyyy HH:mm') : null)} showIcon showTime disabled={disabled} placeholder={placeholder} minDate={minDate} maxDate={maxDate} dateFormat="dd/mm/yy" locale="en" />;
            }
            case 'file': {
                const rules = (typeof field.validation_rule === 'string' ? safeParseJSON(field.validation_rule) : field.validation_rule) || {};
                ...
                return (
                    <div className="d-flex flex-column gap-1">
                        <input
                            ...
                            accept={rules.accept || '*'}
                            onChange={(e) => handleFileFieldUpload(field, e.target.files?.[0] || null)}
            ...
            default: return <InputText className={css} value={val ?? ''} onChange={(e) => { const next = e.target.value; if (typeof field.max_length === 'number' && next.length > field.max_length) { return; } handleChange(field.field_code, next); }} disabled={disabled} placeholder={placeholder} />;
        }
    };
                ...
                if (readOnly) {
                    return (
                        <div className="d-flex flex-column gap-1">
                            ...
                            {showErr ? <small className="text-danger d-block">{err}</small> : null}
                        </div>
                    );
                }
                return (
                ...
                <div className="col-12">
                    <div className="rounded-xl p-4 border shadow-sm bg-white">
                        ...
                        {pages.length === 0 ? (
                            ...
                            <div className="d-flex justify-content-start align-items-top process-wrap mt-5">
                                {pages.map((p: any, idx: number) => {
                                    const isCurrent = idx === activePageIndex;
                                    const isCompleted = completedPages.has(idx);
                                    // Colors
                                    const circleBg = isCompleted ? '#2E7D32' : isCurrent ? '#E5E7EB' : '#FFFFFF';
                                    const circleColor = isCompleted ? '#fff' : isCurrent ? '#374151' : '#BBC3C8';
                                    const circleBorder = isCompleted ? '#2E7D32' : isCurrent ? '#9CA3AF' : '#BBC3C8';
                                            ...
                                            <div className="flex-shrink-0">
                                                ...
                                                >
                                                    <div
                                                        className="mt-2"
                                                    >
                                                        <div className="fw-semibold" style={{ color: isCurrent ? '#2E7D32' : '#BBC3C8', lineHeight: 1.25 }}>
                                                            {p.name || `Page ${idx + 1}`}
                                                        </div>
                                                        <small style={{ color: labelColor, transition: 'color 0.2s ease' }}>Step {idx + 1}</small>
                                                    </div>
                                                </button>
                                            </div>
                                            {idx < pages.length - 1 && (
                    ...
                    <div className="border rounded-xl bg-white overflow-hidden" style={{ borderColor: '#e5e7eb' }}>
                        ...
                        <div className="p-4 p-md-4 d-flex flex-column gap-4">
                            {activePage?.categories?.map((cat: any) => (
                                <div key={cat.id} className="border rounded-lg" style={{ borderColor: '#e5e7eb' }}>
                                    <div className="p-4">
