...
export default function ApplicationFormPage() {
    ...
    const effectiveConfig = useMemo(() => {
        ...
        if (!isAgentRegistrationFlow) return config;
    ...
    }, [approvedProjects, config, initialData?.fields, isAgentRegistrationFlow]);
    ...
    const handleEnsureSubmissionId = async ({ values, addMoreValues, currentPageIndex }: any) => {
        const saveResult = await saveProgress(values, addMoreValues, currentPageIndex);
                    ...
                    <div className="modal-dialog modal-dialog-centered modal-lg">
                        <div className="modal-content border-0 shadow-lg">
                            <div className="modal-header">
                                <h5 className="modal-title fw-bold">{popupTitle}</h5>
                            </div>
                            <div className="modal-body">
                                <div className="max-h-[420px] overflow-y-auto text-sm text-gray-700 pe-2 popup-scroll-content">
                                    {popupConfig.sections?.map((section, index) => (
                                    ...
                                    {popupConfig.html && (
                                        <div dangerouslySetInnerHTML={{ __html: popupConfig.html }} />
                                    )}

                                    {!popupConfig.sections?.length && popupConfig.message && (
                                        <div className="whitespace-pre-wrap">{popupConfig.message}</div>
                                    )}
                                </div>
                                <style jsx>{`
                                ...
                                {popupConfig.requiresAcknowledgement && (
                                    <div className="form-check mt-4">
                                        ...
                                        <label className="form-check-label" htmlFor="service-popup-ack">
                                            {popupConfig.acknowledgementText || 'I have read and understood the above information'}
                                        </label>
                                    </div>
                                )}
                            </div>
                            <div className="modal-footer">
                                ...
                                <button
                                    ...
                                    disabled={popupConfig.requiresAcknowledgement && !popupAcknowledged}
                                    onClick={() => {
                                        setShowPopup(false);
                                        setPopupDismissed(true);
                                    }}
            ...
            {(!(isProjectFlow || isAgentRenewalFlow) || selectedProject) && (
                <>
                    <div className="mb-2 d-flex justify-content-between align-items-center">
                        ...
                        {user?.tenantSlug === 'nmc' && (
                            <img src="/img/line-art.png" alt="Invest Uttarakhand" className="" />
                        )}
                    </div>
                    <div>
                        <FormRenderer
